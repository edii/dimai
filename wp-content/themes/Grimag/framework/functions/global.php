...
//                $category = _make_cat_compat($_item);
//                echo "<pre>";
//                var_dump($_item, $category);
//                echo "</pre>";
                $_html .='<section class="widget-list-news" 
                                    id="category-'.$category -> term_id.'">';
                $_html .= '<h3 class="category-title">
                                <a href="'.get_category_link( $category -> term_id ).'">'.  get_cat_name( $category -> term_id ).'</a>
                            </h3>'; 
                ...
                $posts_array = get_posts( $p_args );
//                echo "<pre>";
//                var_dump( $posts_array );
//                echo "</pre>";
                if(is_array($posts_array) and count($posts_array) > 0) {
                    foreach ($posts_array as $_k => $_post) {
                        ...
                        if(has_post_thumbnail($_post ->ID)) :
                            $_img = get_the_post_thumbnail( $_post->ID, 'thumbnail' ); 
                        endif;
                        
                    $_html .= '<article class="post-list-news post-'. $_post -> ID .' list-post-10">
                                    <div class="entry-thumbnail"> 
                                        <a class="" href="'. get_permalink( $_post -> ID ) .'"> 
                                            '.$_img.'
                                        </a>
                                    </div>
                                    <div class="entry-meta"> 
                                        <span>'.$_post -> post_date.'</span>
                                    </div>
                                    <h2 class="entry-title">
                                        <a title="'. $_post -> post_title .'" 
                                           href="'. get_permalink( $_post -> ID ) .'">
                                            '. $_post -> post_title .'
                                        </a>
                                    </h2>
    ...
    add_action( 'wp_list_category_posts', 'list_category_posts' );
    
    function get_pages_list($_catID) {
        global $wpdb;
