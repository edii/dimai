			...
			<fieldset class="panel-fieldset metabox-fieldset"><legend><?php _e( 'Posts', 'stkit' ) ?></legend>
				...
				<?php
					...
					/*if ( $st_Options['global']['post-formats']['enabled'] ) { ?>
						...
						<select name="<?php echo $prefix . '[' . $number; ?>][format]">
							<option value="all" <?php if ( $format == 'all' ) echo 'selected'; ?>><?php _e( 'All', 'stkit' ); ?></option>
							<?php
			
								foreach ( $st_Options['global']['post-formats'] as $value ) {

									if ( $value['status'] ) {
			
										$out = '<option value="' . $value['label'] . '"';
				
										if ( $format == $value['label'] )
											$out .= ' selected';
				
										$out .= '>' . $value['label'] . '</option>';
						...
						<p><small><?php _e( 'Select a format of posts.', 'stkit' ); ?></small></p>
						<div class="clear"><!-- --></div><?php
...
===============================================*/
	...
	function widget_recent_posts_multi( $args, $vars = array() ) {
		...
		$format = !empty( $vars['format'] ) ? $vars['format'] : '';
