...
function breadcrumb_trail( $args = array() ) {
	...
	$defaults = array(
		'separator' => '/',
		'before' => '<span class="breadcrumb-title">' . __( 'Browse:', 'stkit' ) . '</span>',
		'after' => false,
		'front_page' => true,
...
function breadcrumb_trail_get_items( $args = array() ) {
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a','stkit' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s','stkit' ), get_the_time( __( 'i','stkit' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a','stkit' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y','stkit' ) ) . '">' . get_the_time( __( 'Y','stkit' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F','stkit' ) ) . '">' . get_the_time( __( 'F','stkit' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'd','stkit' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y','stkit' ) ) . '">' . get_the_time( __( 'Y','stkit' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s','stkit' ), get_the_time( esc_attr__( 'W','stkit' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y','stkit' ) ) . '">' . get_the_time( __( 'Y','stkit' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F','stkit' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y','stkit' ) );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;','stkit' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found','stkit' );
...
function breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '' ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y','stkit' ), $post_id ) . '">' . get_the_time( __( 'Y','stkit' ), $post_id ) . '</a>';

			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y','stkit' ), $post_id ) . '">' . get_the_time( __( 'F','stkit' ), $post_id ) . '</a>';

			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y','stkit' ), $post_id ) . '">' . get_the_time( __( 'd','stkit' ), $post_id ) . '</a>';
