...
===============================================*/
	...
	if ( $st_Options['shortcodes']['button'] ) {
		...
		function st_button( $atts, $content = null ) {
			...
			return '<a class="' . $class . '" href="' . $url . '" target="_' . $target . '"' . $style . '><span>' . $content . '</span></a>';
	...
	if ( $st_Options['shortcodes']['alert'] ) {
		...
		function st_alert ( $atts, $content = null ) {
			...
			return '<div class="alert alert-' . $type . '"><span class="alert-before"></span><span class="alert-after"></span><div class="alert-wrapper"><strong>' . $pre[$type] . '</strong> ' . $content . '<span class="ico-st alert-close"><!-- --></span></div><div class="clear"><!-- --></div></div>';
		...
		function st_gravatar( $atts, $content = null ) {
			...
			extract(
				shortcode_atts(
					array(
						'size'	=> '100',
						'email'	=> get_option( 'admin_email', 'doe@stkit.net' ),
						'class'	=> '',
					),
					$atts
				)
