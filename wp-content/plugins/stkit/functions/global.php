...
===============================================*/
	...
	if ( !empty( $st_Options['compatibility']['wp-review'] ) ) {
...
===============================================*/
	...
	if ( !empty( $st_Settings['lifestream'] ) && $st_Settings['lifestream'] == 'yes' || !empty($st_Options['widgets']['social-icons']) ) {
		...
		function st_icons_social( $id = true, $tooltip = true ) {
					...
					foreach ( $st_Options['networks'] as $value ) {
						if ( $st_Settings[$value] ) {
							...
							if ( $id == true ) {
								...
								$attr = 'id="icon-' . $value . '"' . ( $tooltip == true ? ' class="tooltip"' : '' );

							}

							else {

								$attr = 'class="st-icon-' . $value . ( $tooltip == true ? ' tooltip"' : '"' );

							}

							$out .= "\n";
							$out .= '<a ' . $attr . ' title="' . $value . '" href="' . st_get_redirect_page_url() . esc_url( $st_Settings['life_' . $value] ) . '" target="_blank"><!-- --></a>';
	...
	function st_related_posts( $qty = 4, $title = '', $h = 'h6', $image_size = 'project-thumb', $class = 'posts-related-wrapper', $date = false ) {
		...
		global
			...
			if ( get_post_type( $post->ID ) == 'post' ) {
				...
				// get post tags
				$st_['tags'] = array();
				$st_['arr'] = wp_get_post_tags( $post->ID );

				foreach ( $st_['arr'] as $st_['tag'] => $st_['key'] )
					$st_['tags'][] = $st_['key']->term_id;
				...
				// if query result is false -> get a posts by category
				...
				$st_['posts_found'] = count( $st_query->posts );
				$st_['posts_found_class_part'] = $st_['sidebar_position'] == 'none' ? 'full' : count( $st_query->posts );
				...
				while ( $st_query->have_posts() ) : $st_query->the_post();  
					...
					$st_['out'] .= '<td><div class="posts-related-post-wrapper' . ( $st_['postcount'] == 1 && $st_['posts_found'] > 2 ? ' first' : ( $st_['postcount'] == $st_['posts_found'] && $st_['posts_found'] > 2 ? ' last' : '' ) )  . '">';
						...
						$st_['out'] .= '<div class="posts-related-details-wrapper"><div>' .
							...
							( $date ? '<div class="meta"><span class="ico16 ico16-calendar">' . ( !empty( $st_Settings['nice_time'] ) && $st_Settings['nice_time'] == 'yes' && function_exists( 'st_niceTime' ) ? st_niceTime( $post->post_date_gmt ) : get_the_time( get_option('date_format'), $post->ID ) ) . '</span></div>' : '' );
				...
				if ( $st_['out'] ) {
					...
					return '<div class="' . $class . ' posts-related-' . $st_['posts_found_class_part'] . '-wrapper posts-related-' . $st_['posts_found'] . '-wrapper">' . $title . '<table><tbody><tr>' . $st_['out'] . '</tr></tbody></table><div class="clear"><!-- --></div></div>';
	...
	function st_post_share( $id = 1, $gp = true, $fb = true, $tw = true, $dg = true, $dl = true, $st = true, $ln = true, $pn = true ) {

		$out = '<div class="st-post-share"><div>' . "\n";

			// Google+
			if ( $gp ) $out .= '<a class="st-ps-gp tooltip no-auto-lightbox" href="https://plus.google.com/share?url=' . get_permalink( $id ) . '" target="_blank" title="' . __( 'Share on Google+', 'savvythemes' ) . '"><!-- Google+ --></a>';

			// Facebook
			if ( $fb ) $out .= '<a class="st-ps-fb tooltip no-auto-lightbox" href="https://www.facebook.com/sharer/sharer.php?u=' . get_permalink( $id ) . '" target="_blank" title="' . __( 'Share on Facebook', 'savvythemes' ) . '"><!-- Facebook --></a>';

			// Twitter
			if ( $tw ) $out .= '<a class="st-ps-tw tooltip no-auto-lightbox" href="https://twitter.com/home?status=' . str_replace( ' ', '%20', get_the_title( $id ) ) . '%20-%20' . get_permalink( $id ) . '" target="_blank" title="' . __( 'Share on Twitter', 'savvythemes' ) . '"><!-- Twitter --></a>';

			// Digg
			if ( $dg ) $out .= '<a class="st-ps-dg tooltip no-auto-lightbox" href="http://digg.com/submit?url=' . get_permalink( $id ) . '&amp;title=' . str_replace( ' ', '%20', get_the_title( $id ) ) . '" target="_blank" title="' . __( 'Share on Digg', 'savvythemes' ) . '"><!-- Digg --></a>';

			// Delicious
			if ( $dl ) $out .= '<a class="st-ps-dl tooltip no-auto-lightbox" href="https://delicious.com/save?url=' . get_permalink( $id ) . '&amp;title=' . str_replace( ' ', '%20', get_the_title( $id ) ) . '" target="_blank" title="' . __( 'Share on Delicious', 'savvythemes' ) . '"><!-- Delicious --></a>';

			// Stumbleupon
			if ( $st ) $out .= '<a class="st-ps-st tooltip no-auto-lightbox" href="http://www.stumbleupon.com/submit?url=' . get_permalink( $id ) . '&amp;title=' . str_replace( ' ', '%20', get_the_title( $id ) ) . '" target="_blank" title="' . __( 'Share on Stumbleupon', 'savvythemes' ) . '"><!-- Stumbleupon --></a>';

			// LinkedIn
			if ( $ln ) $out .= '<a class="st-ps-ln tooltip no-auto-lightbox" href="https://www.linkedin.com/shareArticle?mini=true&amp;url=' . get_permalink( $id ) . '&amp;title=' . str_replace( ' ', '%20', get_the_title( $id ) ) . '" target="_blank" title="' . __( 'Share on LinkedIn', 'savvythemes' ) . '"><!-- LinkedIn --></a>';

			// Pinterest
			if ( $pn && has_post_thumbnail( $id ) ) {

				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $id ), 'large' );

				$out .= '<a class="st-ps-pn tooltip no-auto-lightbox" href="https://pinterest.com/pin/create/button/?url=' . get_permalink( $id ) . '&amp;media=' . $img[0] . '&amp;description=' . str_replace( ' ', '%20', get_the_title( $id ) ) . '" target="_blank" title="' . __( 'Share on Pinterest', 'savvythemes' ) . '"><!-- Pinterest --></a>';
